...
def velo_callback(msg):
    ...
    #cond = hv_in_range(x=np_p[:, 0],y=np_p[:, 1],z=np_p[:, 2],fov=[-45, 45],fov_type='h')
    ...
    if len(arr_bbox.boxes) is not 0:
        ...
        for i in range(0, len(arr_bbox.boxes)):
            ...
            # arr = np.concatenate((arr, np.array([arr_bbox.boxes[i].pose.position.x,arr_bbox.boxes[i].pose.position.y, arr_bbox.boxes[i].pose.position.z, arr_bbox.boxes[i].dimensions.x,arr_bbox.boxes[i].dimensions.y,arr_bbox.boxes[i].dimensions.z])))


        #  publish to /voxelnet_arr_bbox
        #print(arr_bbox.boxes.)
        pub_arr_bbox.publish(arr_bbox)
        # pub_arr_bbox.publish(arr)
...
if __name__ == '__main__':
    ...
    #sub_ = rospy.Subscriber("/apollo/sensor/velodyne64/compensator/PointCloud2", PointCloud2, velo_callback, queue_size=1)
    ...
    # pub_arr_bbox = rospy.Publisher("voxelnet_arr_bbox", Float64MultiArray, queue_size=10)
